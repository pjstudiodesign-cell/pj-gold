...
# --- 4. GERAÇÃO DE DOCUMENTOS (RESTAURAÇÃO CIRÚRGICA) ---
def gerar_pdf(tipo, p, c):
    pdf = FPDF()
    pdf.add_page()
    
    # Cabeçalho Ouro PJ Studio
    ...
    if tipo == "CONTRATO":
        ...
        pdf.cell(0, 6, f"Endereço do Cliente: {p.get('endereco_cliente', 'N/A')}", ln=True) # CORREÇÃO: ENDEREÇO RESTAURADO
...
if menu == "PAINEL":
    ...
    with st.form("f_orc"):
        ...
        end_cli = st.text_input("Endereço do Cliente") # CORREÇÃO: CAMPO REINSTALADO
        proj = st.text_input("Projeto")
        exig = st.text_input("Exigências")
        val = st.number_input("Valor Total", step=0.01)
        if st.form_submit_button("SALVAR"):
            supabase.table("projetos").insert({"cliente":nome, "cpf_cnpj":doc, "whatsapp_cliente":zap, "endereco_cliente":end_cli, "nome_projeto":proj, "exigencias":exig, "valor_total":val, "status_total":"Pendente", "status_entrada":"Pendente", "status_final":"Pendente"}).execute()
            st.rerun()

...
st.markdown("""
    ...
    /* ESTILIZAÇÃO DO MENU (TABS) NO TOPO */
    ...
    div[data-testid="stMetricLabel"] { color: #FFFFFF !important; font-size: 1.1rem; }
...
# --- INTERFACE (ORDEM PRIORITÁRIA: MENU NO TOPO) ---
...
with tabs[0]:
    # 2. MÉTRICAS LOGO ABAIXO DO MENU NA ABA GESTÃO
    col_m1, col_m2 = st.columns(2)
    col_m1.metric("💰 DINHEIRO NO BOLSO", f"R$ {no_bolso:,.2f}")
    col_m2.metric("⏳ CONTAS A RECEBER", f"R$ {a_receber:,.2f}")
    st.write("---")
    ...
    for p in projetos:
        with st.expander(f"📌 {p.get('nome_projeto')} | CLIENTE: {p.get('cliente')}"):
            ...
            if pdf_orc: b2.download_button("📄 PDF", pdf_orc, f"Orc_{p['id']}.pdf", key=f"pdf_{p['id']}")

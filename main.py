...
# --- CSS PRETO E OURO PREMIUM (FOCO CIRÚRGICO NAS LETRAS DO MENU) ---
st.markdown("""
    <style>
    /* Fundo Total */
    ...
    /* COR OURO NAS LETRAS DO MENU (RADIO BUTTONS) */
    ...
    /* Métricas (Cards Ouro conforme imagem) */
...
if menu == "PAINEL":
    ...
    with st.form("job_form"):
        ...
        if st.form_submit_button("SALVAR"):
            supabase.table("projetos").insert({"nome_projeto": n_p, "cliente": c_p, "valor_total": v_p, "status_integral": "Pendente", "status_entrada": "Pendente", "status_final": "Pendente"}).execute()
            st.rerun()
    ...
    for p in projetos:
        with st.expander(f"📌 {p.get('nome_projeto')} | {p.get('cliente')}"):
            ...
            s_ent = c2.selectbox("Entrada", ["Pendente", "Recebido"], index=0 if p.get('status_entrada')=='Pendente' else 1, key=f"ent_{p['id']}")
            s_fin = c3.selectbox("Final", ["Pendente", "Recebido"], index=0 if p.get('status_final')=='Pendente' else 1, key=f"fin_{p['id']}")
    ...
    with st.form("cfg"):
        ...
        if st.form_submit_button("SALVAR"):
            supabase.table("configuracoes").update({"nome_empresa": n_e}).eq("id", 1).execute()
            st.rerun()

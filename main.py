...
# --- 1. CONFIGURAÇÃO E BLINDAGEM VISUAL (IDENTIDADE PJ STUDIO) ---
st.set_page_config(page_title="PJ STUDIO GOLD PRO", layout="wide")

# --- 2. CONEXÃO SUPABASE (DADOS DO SEU BANCO) ---
...
# --- 3. CSS PRETO E OURO (LACRADO E PROTEGIDO) ---
st.markdown("""
    <style>
    ...
    .stSuccess { background-color: #052205 !important; color: #90ee90 !important; border: 1px solid #90ee90; }
    </style>
    """, unsafe_allow_html=True)

# --- 4. FUNÇÕES DE CARREGAMENTO (ESTABILIDADE) ---
def carregar_dados():
    try:
...
# --- 5. MENU LATERAL (ESTRUTURA APROVADA) ---
with st.sidebar:
...
if menu == "PAINEL":
    ...
    # Uso de form para blindar contra disparos acidentais do "Enter"
    with st.form("orcamento_form", clear_on_submit=False):
        ...
        st.write("")
        submit = st.form_submit_button("GERAR E SALVAR ORÇAMENTO")

        if submit:
            if c_nome and p_nome:
                # Dicionário completo conforme as colunas criadas no SQL
                dados_projeto = {
                    "cliente": c_nome,
                    "cpf_cnpj": c_doc,
                ...
                try:
                    supabase.table("projetos").insert(dados_projeto).execute()
                    st.success("✅ Orçamento salvo com sucesso na PJ STUDIO GOLD!")
                except Exception as e:
                    st.error(f"Erro ao salvar: {e}")
            else:
                st.warning("Atenção: Nome do Cliente e Nome do Projeto são obrigatórios.")

elif menu == "GESTAO DE PROJETOS":
    st.title("📋 GESTÃO DE PROJETOS")
    projetos, _ = carregar_dados()
    if not projetos:
        st.info("Nenhum orçamento ou projeto encontrado.")
    else:
        for p in projetos:
            with st.expander(f"📌 {p.get('nome_projeto')} | {p.get('cliente')}"):
                col_a, col_b = st.columns(2)
                col_a.write(f"**Valor:** R$ {p.get('valor_total', 0)}")
                col_a.write(f"**Prazo:** {p.get('prazo', 'Não definido')}")

...
# 2. Estilo Visual PJ GOLD (Preto e Dourado)
def aplicar_estilo():
    st.markdown("""
        <style>
...
# 4. Funções de PDF (Ajustadas com o novo padrão SaarteSvm)
def gerar_pdf_orcamento(cliente, servico, valor, pgto, prazo, rev, obs):
    try:
        ...
        pdf.ln(10); pdf.set_font("Arial", 'B', 14); pdf.cell(0, 10, "1. ESCOPO DO PROJETO", ln=True)
        ...
        pdf.set_font("Arial", '', 11); pdf.cell(0, 8, f"- Entrega: {prazo} | Alteracoes: {rev}", ln=True)
...
def gerar_pdf_recibo(cliente, servico, valor):
    try:
        ...
        texto = f"Confirmamos o recebimento de R$ {valor:,.2f} de {str(cliente).upper()} referente a: {servico}."
...
def main():
    ...
    if escolha == "Painel":
        st.title(f"💰 Painel Financeiro {info_sidebar[0]}")
        ...
        with col1: st.metric("Total Liquidado", f"R$ {total_rec:,.2f}")
        with col2: st.metric("Em Aberto", f"R$ {total_pend:,.2f}")
        ...
        with st.form("orc_form"):
            ...
            v = st.number_input("Investimento Total", min_value=0.0, step=0.01)
            ser = st.text_area("Escopo do Serviço"); obs_in = st.text_input("Observações Internas")
            c3, c4, c5 = st.columns(3); prz = c3.text_input("Prazo", "7 dias")
            rev = c4.selectbox("Revisões", ["1", "2", "3", "Ilimitadas"])
            pag = c5.text_input("Pagamento", "Entrada + Entrega")
            if st.form_submit_button("CADASTRAR PROJETO"):
                if n and ser:
                    ...
                    conn.commit(); st.success("Projeto cadastrado com sucesso!")
        ...
        for _, r in df.iterrows():
            with st.expander(f"⚜️ {r['cliente']} | R$ {r['valor']:.2f}"):
                ...
                if c_del.button("Remover", key=f"del{r['id']}"):
                    cursor.execute("DELETE FROM projetos WHERE id=?", (r['id'],)); conn.commit(); st.rerun()

    elif escolha == "Configurações":
        st.title("⚙️ Dados da PJ Gold")
        info_f = buscar_dados_empresa()
        with st.form("form_config"):
            nome_emp = st.text_input("Nome Comercial", info_f[0])
            slogan_emp = st.text_input("Slogan / Tagline", info_f[1])
            whats_emp = st.text_input("WhatsApp Profissional", info_f[2])
            email_emp = st.text_input("E-mail de Contato", info_f[3])
            end_emp = st.text_area("Endereço Fiscal/Comercial", info_f[4])
            if st.form_submit_button("SALVAR ALTERAÇÕES"):
                ...
                conn.commit(); st.success("Perfil atualizado!"); st.rerun()
    conn.close()

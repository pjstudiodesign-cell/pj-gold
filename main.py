...
# --- 1. CONFIGURAÇÃO E BLINDAGEM VISUAL (RESTAURADA) ---
st.set_page_config(page_title="PJ STUDIO GOLD PRO", layout="wide")

# --- 2. CONEXÃO SUPABASE (ESTÁVEL) ---
...
# --- 3. CSS PRETO E OURO (COMPLETO E RECUPERADO) ---
st.markdown("""
    <style>
    ...
    .stButton>button { background-color: #D4AF37 !important; color: black !important; font-weight: bold !important; border-radius: 10px !important; width: 100%; }
...
def gerar_pdf(tipo, p, c):
    ...
    if tipo == "CONTRATO":
        ...
        pdf.multi_cell(0, 6, f"CONTRATADO: {c.get('nome_empresa')} | {c.get('email')} | WhatsApp: {c.get('whatsapp')}")
        pdf.ln(4)
        clausulas = [
            ...
            "7. VALIDADE: Ativo após assinatura."
        ]
...
if menu == "PAINEL":
    ...
    c2.metric("⏳ CONTAS A RECEBER", f"R$ {(total - no_bolso):,.2f}")
    ...
    with st.form("orc_form"):
        ...
        p_prazo = col4.text_input("Prazo")
        p_desc = st.text_area("Descrição")
        if st.form_submit_button("GERAR E SALVAR"):
            supabase.table("projetos").insert({"cliente":c_nome, "cpf_cnpj":c_doc, "whatsapp_cliente":c_zap, "nome_projeto":p_nome, "valor_total":p_valor, "prazo":p_prazo, "descricao":p_desc, "status_total":"Pendente", "status_entrada":"Pendente", "status_final":"Pendente"}).execute()
            st.rerun()
    ...
    for p in projetos:
        with st.expander(f"📌 {p.get('nome_projeto')} | {p.get('cliente')}"):
            ...
            n_p = col_ed1.text_input("Projeto", p.get('nome_projeto'), key=f"np_{p['id']}")
            n_c = col_ed2.text_input("Cliente", p.get('cliente'), key=f"nc_{p['id']}")
            st.write("---")
            f1, f2, f3 = st.columns(3)
            v_t = f1.selectbox("TOTAL", ["Pendente", "Recebido"], index=0 if p.get('status_total')=="Pendente" else 1, key=f"vt_{p['id']}")
            v_e = f2.selectbox("ENTRADA", ["Pendente", "Recebido"], index=0 if p.get('status_entrada')=="Pendente" else 1, key=f"ve_{p['id']}")
            v_f = f3.selectbox("FINAL", ["Pendente", "Recebido"], index=0 if p.get('status_final')=="Pendente" else 1, key=f"vf_{p['id']}")
            st.write("---")
    ...
    with st.form("cfg"):
        n_e = st.text_input("Nome Empresa", config.get('nome_empresa', ''))
        c_e = st.text_input("CNPJ", config.get('cpf_cnpj', ''))
        w_e = st.text_input("WhatsApp", config.get('whatsapp', ''))
        e_e = st.text_input("E-mail", config.get('email', ''))
        if st.form_submit_button("SALVAR"):
            supabase.table("configuracoes").update({"nome_empresa":n_e, "cpf_cnpj":c_e, "whatsapp":w_e, "email":e_e}).eq("id", 1).execute(); st.rerun()

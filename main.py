...
# --- 3. CSS PRETO E OURO (MENU E INTERFACE) ---
st.markdown("""
    <style>
...
# --- 4. GERAÇÃO DE DOCUMENTOS (RESTABELECIMENTO TOTAL) ---
def gerar_pdf(tipo, p, c):
    pdf = FPDF()
    pdf.add_page()
    
    # Cabeçalho Profissional PJ Studio
    ...
    if tipo == "CONTRATO":
        ...
        pdf.write(6, f"CONTRATANTE: {p.get('cliente')} | CPF/CNPJ: {p.get('cpf_cnpj', 'N/I')}\n\n")
        clausulas = [
            f"1. OBJETO: Prestação de serviços de design gráfico ({p.get('descricao')}).",
            f"2. PRAZO: {p.get('prazo', 'A combinar')}, iniciando após confirmação do pagamento da entrada.",
            f"3. VALOR E PAGAMENTO: R$ {float(p.get('valor_total', 0)):,.2f} (50% entrada / 50% entrega).",
        ...
        pdf.ln(5)
        pdf.set_font("Arial", 'B', 12); pdf.cell(0, 10, f"VALOR TOTAL: R$ {float(p.get('valor_total', 0)):,.2f}", align='R')
        ...
        if p.get('status_total') == 'Recebido':
            ...
            valor_doc = valor_base
        elif p.get('status_final') == 'Recebido':
            txt = f"PAGAMENTO FINAL (50%) do projeto {p.get('nome_projeto')}."
            valor_doc = valor_base / 2
        else:
            txt = f"ENTRADA (50%) do projeto {p.get('nome_projeto')}."
            valor_doc = valor_base / 2
            
        pdf.multi_cell(0, 7, f"Recebemos a importância de R$ {valor_doc:,.2f} referente à {txt}")
        pdf.ln(10)
        pdf.set_font("Arial", 'B', 12); pdf.cell(0, 10, f"VALOR: R$ {valor_base:,.2f}", align='R')
...
if menu == "PAINEL":
    ...
    with st.form("f_orc"):
        ...
        if st.form_submit_button("SALVAR"):
            supabase.table("projetos").insert({"cliente":nome, "cpf_cnpj":doc, "whatsapp_cliente":zap, "nome_projeto":proj, "exigencias":exig, "valor_total":val, "status_total":"Pendente", "status_entrada":"Pendente", "status_final":"Pendente"}).execute()
            st.rerun()
    ...
    for p in projs:
        with st.expander(f"📌 {p['nome_projeto']} - {p['cliente']}"):
            ...
            v_t = f1.selectbox("TOTAL", ["Pendente", "Recebido"], index=0 if p['status_total']=="Pendente" else 1, key=f"t_{p['id']}")
            v_e = f2.selectbox("ENTRADA (50%)", ["Pendente", "Recebido"], index=0 if p['status_entrada']=="Pendente" else 1, key=f"e_{p['id']}")
            v_f = f3.selectbox("FINAL (50%)", ["Pendente", "Recebido"], index=0 if p['status_final']=="Pendente" else 1, key=f"f_{p['id']}")
            if st.button("💾 ATUALIZAR", key=f"up_{p['id']}"):
                supabase.table("projetos").update({"status_total":v_t, "status_entrada":v_e, "status_final":v_f}).eq("id", p['id']).execute(); st.rerun()
            b1, b2, b3, b4 = st.columns(4)
            b1.download_button("📄 ORÇAMENTO", gerar_pdf("ORC", p, config), f"Orc_{p['id']}.pdf", key=f"bo_{p['id']}")
            b2.download_button("🧾 RECIBO", gerar_pdf("REC", p, config), f"Rec_{p['id']}.pdf", key=f"br_{p['id']}")
            b3.download_button("📜 CONTRATO", gerar_pdf("CONTRATO", p, config), f"Con_{p['id']}.pdf", key=f"bc_{p['id']}")
